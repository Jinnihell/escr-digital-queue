...
export const confirmAppointment = async (appointmentId: string): Promise<void> => {
  ...
  // Generate queue ticket for the confirmed appointment
  try {
    const ticket = await createTicket(
      appointmentData.transactionTypeId,
      appointmentData.transactionTypeName,
      '', // Prefix will be generated by createTicket function
      false,
      appointmentData.userId || null,
      {
        name: appointmentData.studentName,
        studentId: appointmentData.studentId || undefined,
        course: appointmentData.course || undefined,
        yearLevel: appointmentData.yearLevel || undefined,
    ...
    console.error('Error generating ticket for appointment:', err);
