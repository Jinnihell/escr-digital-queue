...
import { getTransactionTypes, initializeDefaultTransactions, initializeDefaultWindows, subscribeToActiveTickets, cancelTicket } from '../services/queueService';
...
export default function TransactionSelection() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToActiveTickets((tickets) => {
      ...
      if (userTicket) {
        ...
        // Get window info if being served
        if (userTicket.status === 'serving' && userTicket.windowId) {
          // Try to get window number from the ticket
          if (userTicket.windowName) {
            const windowMatch = userTicket.windowName.match(/Window\s*(\d+)/i);
            if (windowMatch) {
              setCurrentWindow(parseInt(windowMatch[1]));
            }
          }
        }
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-green-100 via-blue-50 to-blue-200 p-4">
            ...
            <div className="flex justify-center">
              <button
                ...
                className={`px-12 py-3 rounded-xl font-bold text-lg transition-all duration-300 ${
                  selectedId
                    ? 'bg-linear-to-r from-green-500 to-green-600 hover:linear-from-green-600 hover:linear-to-green-700 text-white shadow-lg hover:shadow-xl hover:-translate-y-1'
