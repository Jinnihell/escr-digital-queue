...
export default function StaffDashboard() {
  ...
  const [allWaitingTickets, setAllWaitingTickets] = useState<QueueTicket[]>([]);
  const [currentTicket, setCurrentTicket] = useState<QueueTicket | null>(null);
  const [stats, setStats] = useState<QueueStats | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [showOtherTransactions, setShowOtherTransactions] = useState(false);
    ...
    try {
      ...
      if (currentWindow) {
        filteredTransactions = activeTransactions.filter(
          t => t.windowNumber === currentWindow.number
        );
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('Error parsing window data:', err);
      navigate('/window-selection');
    }
  }, [loadData, navigate]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToActiveTickets((tickets) => {
      ...
      // Get all waiting tickets for the window's primary transactions
      const primaryTransactions = transactions.filter(t => t.windowNumber === selectedWindow?.number);
      const primaryTransactionIds = primaryTransactions.map(t => t.id);
      const allWaiting = tickets.filter(t => 
        t.status === 'waiting' && primaryTransactionIds.includes(t.transactionTypeId)
      );
    ...
    return () => unsubscribe();
  }, [selectedTransaction, selectedWindow, transactions, navigate]);
  ...
  const playNotificationSound = () => {
    ...
    try {
      ...
      // Close audio context after use to prevent memory leak
      setTimeout(() => {
        audioContext?.close();
      }, 1100);
    } catch {
        ...
        <div className="flex flex-wrap gap-6">
              ...
              <div className="text-center">
                ...
                <div className="flex flex-col gap-3">
                  <button
                    ...
                    className="bg-linear-to-r from-blue-800 to-blue-600 hover:linear-from-blue-700 hover:linear-to-blue-500 text-white font-bold py-4 px-8 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transition-all"
                  >
                    📞 Call Next Ticket
                  </button>
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      disabled={!currentTicket}
                      className="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
                    >
                      ⏱️ No Show
                    </button>
                    <button
                      ...
                      disabled={!currentTicket}
                      className="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-2 rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
                    >
                      ✕ Cancel
                    </button>
                  </div>
          ...
          <div className="w-full md:w-72">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-5 mb-6">
              ...
              {/* Waiting count for primary transaction */}
              <div className="mt-3 bg-orange-100 rounded-lg p-2 text-center">
                <p className="text-sm text-gray-600">Waiting for {transactions.find(t => t.id === selectedTransaction)?.name}</p>
                <p className="text-2xl font-bold text-orange-600">{waitingTickets.length}</p>
              </div>
              
              {/* Call Other Transactions Button */}
              {allTransactions.filter(t => t.windowNumber !== selectedWindow?.number).length > 0 && (
                <button
                  onClick={() => setShowOtherTransactions(!showOtherTransactions)}
                  className="mt-3 w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg transition-all"
                >
                  📞 Call Other Window's Queue
                </button>
              )}
              
              {/* Other Transactions Dropdown */}
              {showOtherTransactions && (
                <div className="mt-3 border-2 border-purple-300 rounded-lg p-3 bg-purple-50">
                  <p className="text-sm font-bold text-purple-800 mb-2">Call from other transaction queues:</p>
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {allTransactions
                      .filter(t => t.windowNumber !== selectedWindow?.number)
                      .map((t) => {
                        const otherWaitingCount = allWaitingTickets.filter(wt => wt.transactionTypeId === t.id).length;
                        return (
                          <button
                            key={t.id}
                            onClick={() => {
                              setSelectedTransaction(t.id);
                              setShowOtherTransactions(false);
                            }}
                            className="w-full bg-white hover:bg-purple-100 border border-purple-200 text-purple-800 font-semibold py-2 px-3 rounded-lg text-left flex justify-between items-center"
                          >
                            <span>{t.name}</span>
                            <span className="bg-purple-600 text-white text-xs px-2 py-1 rounded-full">
                              {otherWaitingCount} waiting
                            </span>
                          </button>
                        );
                      })}
